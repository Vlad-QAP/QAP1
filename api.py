...
class PetFriends:
    ...
    def get_list_of_pets_with_invalid_key(self, auth_key: json, filter: str = "") -> json:
        """Метод делает запрос к API сервера с невалидным ключом и возвращает статус запроса с ошибкой.
        На данный момент фильтр может иметь либо пустое значение - получить список всех питомцев, либо 'my_pets' -
        получить список собственных питомцев"""
        headers = {'auth_key': '4092c39cd7071046423f88c959d9162f90aff45a8ae664cb4eb09a98'}
        filter = {'filter': filter}
